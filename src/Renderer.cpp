    ...
    const xsimd::simd_type<float>& two) const
{
    ...
    for (int iter = 0; iter < Parameters::MAX_ITERATIONS; ++iter)
    {
        ...
        zy = two * zx * zy + c_im;
        zx = temp;
        iterations = iterations + xsimd::select(mask, xsimd::batch<float, xsimd::default_arch>(1.0f), xsimd::batch<float, xsimd::default_arch>(0.0f));
    }
    return iterations;
}
...
void Renderer::compute_scalar_pixel(int x, int y, float cy)
{
    ...
    for (; iter < Parameters::MAX_ITERATIONS; ++iter)
    {
        float zx2 = zx * zx;
        float zy2 = zy * zy;
        if (zx2 + zy2 >= Parameters::ESCAPE_RADIUS_SQ)
            break;
